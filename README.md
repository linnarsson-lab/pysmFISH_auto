...
`pysmFISH` is a python package used to analyse data generated by the Linnarsson Lab automated systems (called `ROBOFISH`).

The data can be analysed on a `local` computer or on a `HPC` cluster. The cluster can be `not managed` or managed by[`HTCondor`](https://research.cs.wisc.edu/htcondor/) and has a shared file system.
