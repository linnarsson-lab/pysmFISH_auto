...
The data can be analysed on a `local` computer or on an `HPC` cluster. The cluster we are using is managed by `HTCondor` and has a shared file system. **XXX** **Check if example (with step by step) can be run on coiled cluster**
