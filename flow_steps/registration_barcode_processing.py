...
def registration_barcode_detection_basic(processing_grps,
    ...
    registered_counts_df, all_rounds_shifts, status = calculate_shift_hybridization_fov(processing_grps[0],analysis_parameters)
    
    # Register the fish dots coords
    registered_fish_df, status = register_fish(processing_grps[1],analysis_parameters,registered_counts_df,all_rounds_shifts,status)
