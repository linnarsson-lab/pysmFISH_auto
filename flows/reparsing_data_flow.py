...
with Flow("parsing",environment=LocalEnvironment(DaskExecutor(address='tcp://193.10.16.58:12270')),
    ...
    nd2_selector = nd2_raw_files_selector_general()
    all_raw_nd2 = nd2_selector(folder_fpath=raw_files_fpath)
    all_raw_nd2.set_upstream([raw_files_fpath])
    ...
    nikon_reparser = nikon_nd2_reparser_zarr(task_run_name=lambda **kwargs: f"parsing-{kwargs['nd2_file_path']}")
    parsed_data = nikon_reparser.map(nd2_file_path=all_raw_nd2,parsed_raw_data_fpath=unmapped(parsed_raw_data_fpath),
