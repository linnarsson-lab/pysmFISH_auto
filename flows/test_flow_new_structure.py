...
class create_shoji_db_task(Task):
    def run(self,experiment_info):
        """
        ...
        try:
            ...
            if experiment_name not in db.FISH:
                ...
                ws = db.FISH[experiment_name]
                self.logger.info(f'The experiment was already present')
...
a = list(range(10))
with Flow("new-flow",environment=LocalEnvironment(DaskExecutor(address='tcp://193.10.16.58:32833'))) as flow:
    shoji_db = create_shoji_db_task(task_run_name='create-shoji-db')
    sj = shoji_db(experiment_info)

    # sj = test_sj(task_run_name='create-shoji-db')
    # sj(experiment_info)
