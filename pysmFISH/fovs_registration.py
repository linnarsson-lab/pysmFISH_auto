...
def combine_register_filtered_images(
    output_dict: dict, metadata: dict, all_rounds_shifts: dict
) -> np.ndarray:
        ...
        for channel, all_rounds_data in output_dict.items():
            img_stack = np.zeros(
                [
                    ...
                    int(metadata["img_width"]),
                    int(metadata["img_height"]),
                ]
            )
...
def combine_register_filtered_image_single_channel(
    output_dict: dict, metadata: dict, all_rounds_shifts: dict
) -> np.ndarray:
    """Function used to register the image throughout all rounds.
...
def identify_matching_register_dots_NN(
    ref_dots_coords_fov, tran_registered_coords, registration_tollerance_pxl
):
...
def beads_based_registration(
    all_counts_fov: pd.DataFrame, analysis_parameters: Dict
) -> pd.DataFrame:
    """[summary]
    ...
    stitching_channel_df = all_counts_fov.loc[
        (all_counts_fov.channel == stitching_channel)
    ...
    if stitching_channel_df[stitching_channel_df["dot_id"].isnull()].empty:
        ...
        ref_counts_df = stitching_channel_df.loc[
            stitching_channel_df.round_num == reference_round_num, :
        ]

        all_counts_fov.loc[ref_counts_df.index, "r_px_registered"] = ref_counts_df.loc[
            ref_counts_df.round_num == reference_round_num, "r_px_original"
        ]
        all_counts_fov.loc[ref_counts_df.index, "c_px_registered"] = ref_counts_df.loc[
            ref_counts_df.round_num == reference_round_num, "c_px_original"
        ]
        all_counts_fov.loc[ref_counts_df.index, "r_shift_px"] = 0
        all_counts_fov.loc[ref_counts_df.index, "c_shift_px"] = 0
        all_counts_fov.loc[
            ref_counts_df.index, "min_number_matching_dots_registration"
        ] = 1000
        ...
        all_counts_fov.loc[
            fish_ref_round.index, "r_px_registered"
        ] = fish_ref_round.loc[fish_ref_round.index, "r_px_original"]
        all_counts_fov.loc[
            fish_ref_round.index, "c_px_registered"
        ...
        all_counts_fov.loc[
            fish_ref_round.index, "min_number_matching_dots_registration"
        ] = 1000
        ...
        for tran_round_num in all_rounds:
            ...
            tran_counts_df = stitching_channel_df.loc[
                stitching_channel_df.round_num == tran_round_num, :
            ]
            img_width = tran_counts_df.iloc[0]["img_width"]
            img_height = tran_counts_df.iloc[0]["img_height"]
            tran_coords = tran_counts_df.loc[
                :, ["r_px_original", "c_px_original"]
            ].to_numpy()
        ...
        all_counts_fov["c_shift_px"] = np.nan
        all_counts_fov[
            "min_number_matching_dots_registration"
        ] = registration_errors.missing_counts_reg_channel

    return all_counts_fov


def beads_based_registration_stitching_channel(
    stitching_channel_df: pd.DataFrame, analysis_parameters: Dict, metadata: Dict
    ...
    if stitching_channel_df.shape[0]:
        ...
        while True:
            if not ref_counts_df.shape[0]:
                ...
                stitching_channel_df = stitching_channel_df.append(
                    {"round_num": reference_round_num}, ignore_index=True
                )
                ...
                if (
                    reference_round_num
                    == analysis_parameters["RegistrationReferenceHybridization"]
                ):
                    stitching_channel_df.loc[
                        stitching_channel_df.round_num == reference_round_num,
                        "min_number_matching_dots_registration",
                    ] = registration_errors.missing_counts_reference_round
                    ...
                    stitching_channel_df.loc[
                        stitching_channel_df.round_num == reference_round_num,
                        "min_number_matching_dots_registration",
                    ] = registration_errors.missing_counts_in_round
        ...
        if reference_round_num > metadata["total_rounds"]:
            ...
            stitching_channel_df.loc[
                ref_counts_df.index, "r_px_registered"
            ] = ref_counts_df.loc[
                ref_counts_df.round_num == reference_round_num, "r_px_original"
            ]
            stitching_channel_df.loc[
                ref_counts_df.index, "c_px_registered"
            ] = ref_counts_df.loc[
                ref_counts_df.round_num == reference_round_num, "c_px_original"
            ]
            ...
            for tran_round_num in all_rounds:
                ...
                if tran_counts_df.shape[0]:
                    ...
                    stitching_channel_df.loc[
                        tran_counts_df.index, "r_shift_px"
                    ] = shift[0]
                    stitching_channel_df.loc[
                        tran_counts_df.index, "c_shift_px"
                    ] = shift[1]
                    if min_num_matching_dots >= registration_tollerance_counts:
                        stitching_channel_df.loc[
                            tran_counts_df.index,
                            "min_number_matching_dots_registration",
                        ] = min_num_matching_dots
                    else:
                        stitching_channel_df.loc[
                            tran_counts_df.index,
                            "min_number_matching_dots_registration",
                        ] = registration_errors.number_beads_below_tolerance_counts

                else:

                    stitching_channel_df = stitching_channel_df.append(
                        {"round_num": tran_round_num}, ignore_index=True
                    )
                    stitching_channel_df.loc[
                        stitching_channel_df.round_num == tran_round_num,
                        "min_number_matching_dots_registration",
...
def beads_based_registration_fish(
    all_counts_fov: pd.DataFrame,
    ...
    if not all_counts_fov.shape[0]:
        all_counts_fov = all_counts_fov.append(
            {
                "min_number_matching_dots_registration": registration_errors.missing_counts_fish_channel
            },
            ignore_index=True,
        )
        ...
        for round_num, shift in all_rounds_shifts.items():
            if isinstance(shift, np.ndarray):
                all_counts_fov.loc[
                    all_counts_fov.round_num == round_num, "r_px_registered"
                ] = (all_counts_fov["r_px_original"] + shift[0])
                all_counts_fov.loc[
                    all_counts_fov.round_num == round_num, "c_px_registered"
                ] = (all_counts_fov["c_px_original"] + shift[1])
                all_counts_fov.loc[
                    all_counts_fov.round_num == round_num, "r_shift_px"
                ] = shift[0]
                all_counts_fov.loc[
                    all_counts_fov.round_num == round_num, "c_shift_px"
                ] = shift[1]
                all_counts_fov.loc[
                    all_counts_fov.round_num == round_num,
                    "min_number_matching_dots_registration",
                ] = all_rounds_matching_dots[round_num]
            else:
                all_counts_fov.loc[
                    all_counts_fov.round_num == round_num, "r_px_registered"
                ] = np.nan
...
def nuclei_based_registration(
    all_counts_fov: pd.DataFrame, img_stack: np.ndarray, analysis_parameters: Dict
) -> pd.DataFrame:
    ...
    if stitching_channel_df[stitching_channel_df["dot_id"].isnull()].empty:
        ...
        for r_num in all_rounds:
            ...
            shift, error, diffphase = phase_cross_correlation(
                ref_img, img_stack[r_num, :, :], return_error=True
            )
...
def fft_registration_beads(
    ...
    if np.any(np.isnan(reference_coords)):
        ...
        logger.error(
            f"missing registration round {hybridization_num_translated} for registration for fov {fov_num}"
        )
        shift = np.array([np.nan, np.nan])
        error = 0
        tran_registered_coords = translated_coords
...
def calculate_shift_hybridization_fov(
    processing_files: List, analysis_parameters: dict, save=True
):
    """
    Function used to run the registration of a single fov
    through all hybridization. The registration is done using the dots
    ...
    reference_hybridization_str = "Hybridization" + str(reference_hybridization).zfill(
        2
    )
    registration_tollerance_pxl = analysis_parameters["RegistrationTollerancePxl"]
    ...
    fname = (
        ...
        / "tmp"
        / "registered_counts"
        / (experiment_name + "_" + channel + "_registered_fov_" + fov + ".parquet")
    )
    shift_fname = (
        experiment_fpath
        / "tmp"
        / "registered_counts"
        / (experiment_name + "_" + channel + "_all_rounds_shifts_fov_" + fov + ".pkl")
    )
    ...
    try:
        ...
        output_registration_df = output_registration_df.append(
            {
                "min_number_matching_dots_registration": registration_errors.missing_file_reg_channel,
                "fov_num": int(fov),
                "dot_channel": channel,
        ...
        try:
            ...
            output_registration_df = output_registration_df.append(
                {
                    "min_number_matching_dots_registration": registration_errors.cannot_load_file_reg_channel,
                    "fov_num": int(fov),
                    "dot_channel": channel,
                    "round_num": int(round_num),
                },
            ...
            if np.any(np.isnan(ref_counts["r_px_original"])) or (
                ref_counts["r_px_original"].shape[0] < registration_tollerance_pxl
            ):
                logger.error(f"There are no dots in there reference hyb for fov {fov} ")
                output_registration_df = output_registration_df.append(
                    {
                        "min_number_matching_dots_registration": registration_errors.missing_counts_reg_channel,
                        "fov_num": int(fov),
                        "dot_channel": channel,
                        "round_num": int(round_num),
                    },
                ...
                output_registration_df = pd.concat(
                    [output_registration_df, ref_counts_df], axis=0, ignore_index=True
                )
                ...
                for fpath in tran_processing_files:
                    try:
                        ...
                        round_num = int(
                            (fpath.stem).split("_")[-5].split("Hybridization")[-1]
                        )
                        output_registration_df = data_models.output_registration_df
                        output_registration_df = output_registration_df.append(
                            {
                        ...
                        if (
                            np.any(np.isnan(tran_coords))
                            or tran_coords.shape[0] < registration_tollerance_pxl
                        ):
                            round_num = int(
                                (fpath.stem).split("_")[-5].split("Hybridization")[-1]
                            )
                            ...
                            all_rounds_shifts[round_num] = np.array([np.nan, np.nan])
                            logger.error(
                                f" {fpath.stem} file for fov {fov} has no counts"
                            )
                            break
                        else:

                            round_num = tran_counts["round_num"][0]
                            ref_img_metadata["reference_hyb"] = str(
                            ...
                            tran_counts_df["r_px_registered"] = registered_tran_coords[
                                :, 0
                            ]
                            tran_counts_df["c_px_registered"] = registered_tran_coords[
                                :, 1
                            ]
                            ...
                            output_registration_df = pd.concat(
                                [output_registration_df, tran_counts_df],
                                axis=0,
                                ignore_index=True,
                            )
                ...
                output_registration_df["experiment_type"] = ref_img_metadata[
                    "experiment_type"
                ]
                output_registration_df["experiment_name"] = ref_img_metadata[
                    "experiment_name"
                ]
                output_registration_df["pxl_um"] = ref_img_metadata["pixel_microns"]
                output_registration_df["stitching_type"] = ref_img_metadata[
                    "stitching_type"
                ]
                ...
                output_registration_df["fov_acquisition_coords_x"] = ref_img_metadata[
                    "fov_acquisition_coords_x"
                ]
                output_registration_df["fov_acquisition_coords_y"] = ref_img_metadata[
                    "fov_acquisition_coords_y"
                ]
                output_registration_df["fov_acquisition_coords_z"] = ref_img_metadata[
...
def calculate_shift_hybridization_fov_test(
    fov: int, counts_output: Dict, analysis_parameters: Dict, experiment_info: Dict
):
    """
    Function used to run the registration of a single fov
    through all hybridization. The registration is done using the dots
    ...
    try:
        ref_zarr_name = [
            el for el in counts_zarr_names if reference_hybridization_str in el
        ][0]
    except:
        ...
        ref_counts, ref_img_metadata = counts_output["registration"][channel][
            ref_zarr_name
        ]
        # Check if there are dots detected in the reference round
        if np.any(np.isnan(ref_counts["r_px_original"])) or (
            ...
            for zarr_name in tran_processing_files:
                ...
                try:
                    tran_counts, tran_img_metadata = counts_output["registration"][
                        channel
                    ][zarr_name]
                except:
...
def register_fish(
    ...
    if status == "FAILED":
        error = registered_reference_channel_df[
            "min_number_matching_dots_registration"
        ].values[0]
        round_num = registered_reference_channel_df["round_num"].values[0]
        registered_fish_df = registered_fish_df.append(
            {
        ...
        reference_hybridization = registered_reference_channel_df.attrs[fov][
            "reference_hyb"
        ]
        reference_hybridization_str = "Hybridization" + str(
            reference_hybridization
        ).zfill(2)
        for fpath in processing_files:
            ...
            try:
                ...
                registered_fish_df = registered_fish_df.append(
                    {
                        "min_number_matching_dots_registration": registration_errors.cannot_load_file_fish_channel,
                        "fov_num": int(fov),
                        "dot_channel": channel,
                        "round_num": round_num,
                    },
                ...
                if np.any(np.isnan(fish_counts["r_px_original"])):
                    logger.error(
                        f"There are no dots in there reference hyb for fov {fov} "
                    )
                    registered_fish_df = registered_fish_df.append(
                        {
                            "min_number_matching_dots_registration": registration_errors.missing_counts_fish_channel,
                    ...
                    subset_df = registered_reference_channel_df.loc[
                        registered_reference_channel_df["round_num"] == round_num, :
                    ]
                    subset_df = subset_df.reset_index()

                    shift = all_rounds_shifts[round_num]
                    fish_counts_df["r_px_registered"] = (
                        fish_counts["r_px_original"] + shift[0]
                    )
                    fish_counts_df["c_px_registered"] = (
                        fish_counts["c_px_original"] + shift[1]
                    )
                    fish_counts_df["r_shift_px"] = shift[0]
                    fish_counts_df["c_shift_px"] = shift[1]
                    fish_counts_df[
                        "min_number_matching_dots_registration"
                    ] = subset_df.loc[0, "min_number_matching_dots_registration"]
                    ...
                    registered_fish_df = pd.concat(
                        [registered_fish_df, fish_counts_df], axis=0, ignore_index=True
                    )
                    status = "SUCCESS"

    if save:
        fname = (
            file_tags["experiment_fpath"]
            / "tmp"
            / "registered_counts"
            / (
                file_tags["experiment_name"]
                + "_"
                + file_tags["channel"]
                + "_registered_fov_"
                + file_tags["fov"]
...
def register_fish_test(
    ...
    if status == "FAILED":
        ...
        reference_hybridization = analysis_parameters[
        ...
        for zarr_name in counts_zarr_names:
            round_num = int(zarr_name.split("_")[-4].split("Hybridization")[-1])
            try:
                fish_counts, fish_img_metadata = counts_output["fish"][channel][
                    zarr_name
                ]
            except:
                ...
                if np.any(np.isnan(fish_counts["r_px_original"])):
                    ...
                    fish_counts_df["reference_hyb"] = reference_hybridization
                    fish_counts_df["experiment_type"] = subset_df.loc[
                        0, "experiment_type"
                    ]
                    fish_counts_df["experiment_name"] = subset_df.loc[
                        0, "experiment_name"
                    ]
                    fish_counts_df["pxl_um"] = subset_df.loc[0, "pxl_um"]
                    fish_counts_df["stitching_type"] = subset_df.loc[
                        0, "stitching_type"
                    ]
                    fish_counts_df["fov_acquisition_coords_x"] = subset_df.loc[
                        0, "fov_acquisition_coords_x"
                    ]
                    fish_counts_df["fov_acquisition_coords_y"] = subset_df.loc[
                        0, "fov_acquisition_coords_y"
                    ]
                    fish_counts_df["fov_acquisition_coords_z"] = subset_df.loc[
                        0, "fov_acquisition_coords_z"
                    ]
...
def calculate_shift_hybridization_fov_nuclei(
    processing_files: List, analysis_parameters: dict, save=True
):
    """
    Function used to run the registration of a single fov
    ...
    shift_fname = (
        ...
        / (
            ...
            + "_"
            + channel
            + "_nuclei_all_rounds_shifts_fov_"
            + fov
            + ".pkl"
        )
    )
    shift_error_fname = (
        ...
        / (
            ...
            + "_"
            + channel
            + "_nuclei_all_rounds_shifts_errors_fov_"
            + fov
            + ".pkl"
        )
    )
...
def calculate_shift_hybridization_fov_nuclei_test(
    img_stack: np.ndarray, analysis_parameters: Dict
):
    """
    Function used to run the registration of a single fov
    through all hybridization. The registration is done using the dots
...
def register_fish_on_nuclei_test(
    fov: int,
    counts_output: Dict,
    registered_reference_channel_df,
    all_rounds_shifts: Dict,
...
def create_registration_grps(
    experiment_fpath: str, registration_channel: str, fovs: List, save=True
):
    """
    ...
    for fov in fovs:
        ...
        grp_reg = [
            reg_f for reg_f in registration_files if search_key_reg in reg_f.as_posix()
        ]
        grp_fish = [
            fish_f for fish_f in fish_files if search_key_fish in fish_f.as_posix()
        ]
        logger.debug(
            f"fov {fov} for registration on {registration_channel} has {len(grp_reg)} counts files"
        )
...
def determine_overlap_region(self):
    ...
    if np.ma.is_masked(self.micData.tile_set.flat[:][self.ind1]):
        ...
        tile_1 = True
        fnum_tile_1 = (
            self.micData.tile_set.flat[:][self.ind1] + self.micData.tile_nr.min()
        )
    ...
    if np.ma.is_masked(self.micData.tile_set.flat[:][self.ind2]):
        ...
        tile_2 = True
        fnum_tile_2 = (
            self.micData.tile_set.flat[:][self.ind2] + self.micData.tile_nr.min()
        )

    if tile_1 and tile_2:
        self.tiles_num = (fnum_tile_1, fnum_tile_2)
        self.tile1_x_coords = self.micData.x_coords[
            self.micData.tile_set.flat[:][self.ind1]
        ]
        self.tile2_x_coords = self.micData.x_coords[
            self.micData.tile_set.flat[:][self.ind2]
        ]
        self.tile1_y_coords = self.micData.y_coords[
            self.micData.tile_set.flat[:][self.ind1]
        ]
        self.tile2_y_coords = self.micData.y_coords[
            self.micData.tile_set.flat[:][self.ind2]
        ]

        tile_1_fpath = [
            fpath
            for fpath in self.counting_files_list
            if "pos_" + str(fnum_tile_1) + "." in str(fpath)
        ][0]
        self.tile_1_store = zarr.DirectoryStore(tile_1_fpath)
        self.tile_1_root = zarr.group(store=self.tile_1_store, overwrite=False)
        self.tile_1_counts = self.tile_1_root["stringency_raw_counts"][
            "coords_original"
        ][...]
...
class chunking:
    ...
    def block_chunking(self):
        ...
        if nr_chunks == 1:
            ...
            r_coords_tl = np.arange(
                self.starting_position[0],
                (self.starting_position[0] + self.chunk_size * (nr_chunks)),
                self.chunk_size,
            )
        ...
        if nc_chunks == 1:
            ...
            c_coords_tl = np.arange(
                self.starting_position[1],
                (self.starting_position[1] + self.chunk_size * (nc_chunks)),
                self.chunk_size,
            )
        ...
        r_coords_tl_all_padded[r_coords_tl_all_padded < 0] = r_coords_tl_all[
            r_coords_tl_all_padded < 0
        ]
        c_coords_tl_all_padded[c_coords_tl_all_padded < 0] = c_coords_tl_all[
            c_coords_tl_all_padded < 0
        ]
        r_coords_br_all_padded[r_coords_br_all_padded > num_r] = r_coords_br_all[
            r_coords_br_all_padded > num_r
        ]
        c_coords_br_all_padded[c_coords_br_all_padded > num_c] = c_coords_br_all[
            c_coords_br_all_padded > num_c
        ]
        ...
        for r in np.arange(0, r_coords_tl_all_padded.shape[0]):
            for c in np.arange(0, r_coords_tl_all_padded.shape[1]):
                self.Coords_Padded_Chunks_list.append(
                    np.array(
                        [
                            r_coords_tl_all_padded[r][c],
...
class reference_beads_registration:
    ...
    def errors(transformed_coords, ref_good):
        diagonals = np.sqrt(
            (ref_good[:, 0] - transformed_coords[:, 0]) ** 2
            + (ref_good[:, 1] - transformed_coords[:, 1]) ** 2
        )
    ...
    def calculate_NN_roi(self, chunk_coords):
        ...
        # Select only the coords in the trimmed region
        ref_trimmed = self.ref_hyb_coords[
            :,
            (
                (r_tl < self.ref_hyb_coords[0, :])
                & (self.ref_hyb_coords[0, :] < r_br)
                & (c_tl < self.ref_hyb_coords[1, :])
                & (self.ref_hyb_coords[1, :] < c_br)
            ),
        ]
        tran_trimmed = self.comp_hyb_coords[
            :,
            (
                (r_tl < self.comp_hyb_coords[0, :])
                & (self.comp_hyb_coords[0, :] < r_br)
                & (c_tl < self.comp_hyb_coords[1, :])
                & (self.comp_hyb_coords[1, :] < c_br)
            ),
        ]

        # Add check if there are dots in the timmed region
        if (
            ref_trimmed.size >= 2 * self.n_neighbors
            and tran_trimmed.size >= 2 * self.n_neighbors
        ):
            nbrs_ref = NearestNeighbors(
                n_neighbors=self.n_neighbors,
                algorithm="ball_tree",
                radius=self.matching_radius,
            ).fit(ref_trimmed.T)
            nbrs_tr = NearestNeighbors(
                n_neighbors=self.n_neighbors,
                algorithm="ball_tree",
                radius=self.matching_radius,
            ).fit(tran_trimmed.T)
    ...
    def calculate_NN_overlapping_region(self, coords_overlapping_region):
        ...
        # Add check if there are dots in the timmed region
        if (
            coords_overlapping_region.size >= 2 * self.n_neighbors
            and coords_overlapping_region.size >= 2 * self.n_neighbors
        ):
            nbrs = NearestNeighbors(
                n_neighbors=self.n_neighbors,
                algorithm="ball_tree",
                radius=self.matching_radius,
            ).fit(coords_overlapping_region.T)
        else:
            nbrs = np.nan
    ...
    def calculate_registration(self):
        ...
        if self.ref_hyb_coords.size and self.comp_hyb_coords.size:
            for chunk_coords in self.Coords_Padded_Chunks_list:
                ref_trimmed, tran_trimmed, nbrs_ref, nbrs_tr = self.calculate_NN_roi(
                    chunk_coords
                )
                if (
                    ref_trimmed.size >= 2 * self.n_neighbors
                    ...
                    if nbrs_ref != np.nan and nbrs_tr != np.nan:
                        ...
                        for tran_dot_id in trans_dot_id_list:
                            ...
                            dist, idx = nbrs_tr.kneighbors(
                                searching, n_neighbors=self.n_neighbors
                            )
                            ...
                            for id_r in np.arange(ref_trimmed.shape[1]):
                                ...
                                dist_r, idx_r = nbrs_ref.kneighbors(
                                    searching_r, n_neighbors=self.n_neighbors
                                )
                                if idx_r[0].shape[0] == idx[0].shape[0]:
                                    selected_ref = ref_trimmed[:, idx_r[0]]
                                    ref_dist = self.calculate_min_distances(
                                        selected_ref
                                    )
                                    if np.all(
                                        np.abs(ref_dist - tran_dist)
                                        < self.min_acceptable_distance
                                    ):
                                        ...
                                        if passing_num == 0:
                                            ...
                                            matching_points = np.concatenate(
                                                (matching_points, cpls), axis=0
                                            )
            if isinstance(matching_points, np.ndarray):
                ...
                if (self.min_samples < self.ref.shape[0]) and (
                    self.min_samples < self.tran.shape[0]
                ):
                    self.model, self.inliers = ransac(
                        (self.tran, self.ref),
        ...
        if not self.missing_pos:
            if self.model:
                ...
                self.transformed_coords = transform.matrix_transform(
                    self.tr_good, self.model.params
                )
                self.transformed_all_coords = transform.matrix_transform(
                    self.comp_hyb_coords.T, self.model.params
                )
                ...
                self.translation_diagonal = np.sqrt(
                    (self.ref_good[:, 0] - self.tr_good[:, 0]) ** 2
                    + (self.ref_good[:, 1] - self.tr_good[:, 1]) ** 2
                )
                self.translational_diagonal_mean = np.mean(self.translation_diagonal)
                self.translational_diagonal_median = np.median(
                    self.translation_diagonal
                )
                self.translation_diagonal_std = np.std(self.translation_diagonal)
                self.translation_diagonal_sem = np.std(self.translation_diagonal) / len(
                    self.translation_diagonal
                )
                (
                    self.err1,
                    self.err2,
                    self.diagonal_mean,
                    self.diagonal_median,
                    self.diag_std,
                    self.diag_sem,
                ) = self.errors(self.transformed_coords, self.ref_good)
    ...
    def deploy(self):
        ...
        if self.ref_hyb_coords.size >= 2 * self.n_neighbors:
            ...
            if self.comp_hyb_coords.size >= 2 * self.n_neighbors:
                self.comp_nbrs = self.calculate_NN_overlapping_region(
                    self.comp_hyb_coords
                )
        ...
        if self.missing_pos:
            ...
            self.registration_data = {
                "translation_diagonal_mean": self.translational_diagonal_mean,
    ...
    def calculate_registration(self):
                    ...
                    if nbrs_ref != np.nan and nbrs_tr != np.nan:
                        ...
                        for tran_dot_id in trans_dot_id_list:
                            ...
                            for id_r in np.arange(ref_trimmed.shape[0]):
                                ...
                                dist_r, self.idx_r = nbrs_ref.kneighbors(
                                    self.searching_r, n_neighbors=self.n_neighbors
                                )
                                if self.idx_r[0].shape[0] == idx[0].shape[0]:
                                    ...
                                    if np.all(
                                        ...
                                        if passing_num == 0:
                                            ...
                                            self.matching_points = np.concatenate(
                                                (self.matching_points, cpls), axis=0
                                            )
        ...
        if not self.missing_pos:
            if self.model:
                ...
                self.transformed_all_coords = transform.matrix_transform(
                    self.comp_hyb_coords, self.model.params
                )
                self.delta = self.ref_good - self.tr_good
                self.translation_mean = np.mean(self.delta, axis=0)
                self.translation_median = np.median(self.delta, axis=0)
