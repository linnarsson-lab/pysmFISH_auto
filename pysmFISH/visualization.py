...
def visualize_raw_counts(experiment_fpath:str,dataset_name:str,
    ...
    try:
        ...
        counts_fname = fov_dataset.experiment_name.iloc[0] + '  ' + channel + '_fov_' +str(fov_num) + '.parquet'
        ...
        try:
            ...
            for round_num in fov_dataset.round_num.unique():
                ...
                preprocessed_img, metadata = io.load_general_zarr(fov_subdataset,
...
def visualize_barcodes(experiment_fpath:str,dataset_name:str,
                        fov_num:int,channel:str):
    """Utility function to visualize the barcodes for a FOV.
    The preprocessed images for each round are combined with the
    corresponding counts and plot on different napari layers
    ...
    try:
        ...
        counts_fname = fov_dataset.experiment_name.iloc[0] + '_decoded_fov_' +str(fov_num) + '.parquet'
        counts_fpath = experiment_fpath / 'results' / counts_fname
        ...
        try:
            ...
            all_counts_df = all_counts_df.loc[all_counts_df.channel == channel, :]
            vw = napari.Viewer(title='barcodes fov ' + str(fov_num) + 'channel ' + channel)
            counts_coords = all_counts_df.loc[:,['r_px_original','c_px_original']].to_numpy()
            vw.add_points(counts_coords, name= 'counts fov '+ str(fov_num), face_color='cyan',
                            symbol='+', size=5, opacity=0.5,visible=False)
            for round_num in fov_dataset.round_num.unique():
                fov_subdataset = fov_dataset.loc[fov_dataset.round_num == round_num,:].iloc[0]
                preprocessed_img, metadata = io.load_general_zarr(fov_subdataset,
