...
def nikon_nd2_reparser_zarr_custom_dataset(nd2_file_path,parsed_raw_data_fpath,experiment_info):
    ...
    try:
        ...
        hybridization_num = int(hybridization_name.split('Hybridization')[-1])
        for fov in fields_of_view:
            data_info_fname = parsed_raw_data_fpath / (tag_name + 'data_info.pkl') 
            output_attrs = {}
            img = np.array(nd2fh[fov],dtype=np.uint16)      
            array_name = tag_name + '_fov_' + str(fov)
            ...
            root.attrs['fov_name'] = fov_name
            root.attrs['channel'] = channel
            root.attrs['target_name'] = info_data['channels'][channel]
            root.attrs['img_width'] = parsed_metadata['width']
            root.attrs['img_height'] = parsed_metadata['height']
            root.attrs['pixel_microns'] = pixel_microns
            root.attrs['z_levels'] = list(z_levels)
            root.attrs['fields_of_view'] = list(fields_of_view)
            root.attrs['fov_num'] = fov
            root.attrs['stitching_channel'] = info_data['StitchingChannel']
            root.attrs['stitching_type'] = experiment_info['Stitching_type']
            root.attrs['experiment_type'] = experiment_info['Experiment_type']
            root.attrs['hybridization_num'] = hybridization_num
            root.attrs['experiment_name'] = experiment_name
            root.attrs['fov_acquisition_coords_x'] = fov_coords[fov,1]
            root.attrs['fov_acquisition_coords_y'] = fov_coords[fov,2]
            root.attrs['fov_acquisition_coords_z'] = fov_coords[fov,0]
            ...
            output_attrs['z_levels'] = list(z_levels)
            output_attrs['fields_of_view'] = list(fields_of_view)
            ...
            output_attrs['hybridization_num'] = hybridization_num
            ...
            if info_data['StitchingChannel'] == channel:
                ...
                root.attrs['processing_type'] = 'fish'
