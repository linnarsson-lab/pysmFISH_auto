...
def align(target: np.ndarray, source: np.ndarray, radius: float) -> Tuple[np.ndarray, np.ndarray]:
...
def plot_align(dist: np.ndarray, radians: np.ndarray, s: float=5,
               alpha: float=0.001):
...
def gradient_acent(xy: np.ndarray, bandwidth: float=5, search_radius: float=20, max_iterations: int=500, 
                   resolution: float=0.05, rtol: float=1e-3, plot: bool=False, alpha: float=0.1) -> np.ndarray:
    ...
    #Make KDE
    kde = KernelDensity(bandwidth=bandwidth, kernel='gaussian', rtol=rtol, algorithm='ball_tree')
    kde.fit(xy)
    
    #Initial scoring to find starting point
    step = search_radius / 5
    xiyi = np.meshgrid(np.arange(-search_radius, search_radius, step), np.arange(-search_radius, search_radius, step))
    xi = xiyi[0].ravel()
    yi = xiyi[1].ravel()
...
def find_offset(A: np.ndarray, B: np.ndarray, radius: float=20, max_iterations: int=500, bandwidth: float=5, 
                resolution: float=0.05, initial_offset: np.ndarray=np.array([0,0]), rtol: float=1e-3, 
                plot: bool=False, alpha: float=0.1, integer: bool=False) -> np.array:
    ...
    #Find center
    offset = gradient_acent(xy, bandwidth=bandwidth, search_radius=radius, max_iterations=max_iterations, 
                            resolution=resolution, rtol=rtol, plot=plot, alpha=alpha)
    #Flip offset
...
def find_matches(A: np.ndarray, B: np.ndarray, id_A: np.ndarray, id_B: np.ndarray, offset: np.ndarray, 
                 max_radius: int=10) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
...
def select_overlap(rc:np.ndarray, corner_coordinates:Union[list, np.ndarray]) -> np.ndarray:
...
def clean_microscope_stitched(fovs:Union[list, np.ndarray], fov_df:dict, 
    ...
    for i in fovs:
        ...
        if fov_neignbours:
            ...
            for k, v in zip(key, values):
                ...
                fov0, fov1 = pair
                if verbose:
                    print(f'FOV :{i}/{fovs[-1]}. Pair: {fov0} - {fov1}           ', end='\r')

                #Get FOV 0
                d0 = fov_df[fov0]
                #Load all valid RNA signal
                d0_filt_rna = np.logical_and(d0.channel != bead_channel, d0.hamming_distance < max_hamming_dist)
                d0_filt = deepcopy(d0_filt_rna)
                d0_rc = d0.loc[d0_filt, ['r_px_microscope_stitched', 'c_px_microscope_stitched']].to_numpy()
                ...
                d1 = fov_df[fov1]
                #Load all valid RNA signal
                d1_filt_rna = np.logical_and(d1.channel != bead_channel, d1.hamming_distance < max_hamming_dist)
                d1_filt = deepcopy(d1_filt_rna)
                d1_rc = d1.loc[d1_filt, ['r_px_microscope_stitched', 'c_px_microscope_stitched']].to_numpy()
                #Select RNA in overlap
        ...
        #FOV has no neighbours. Just delete beads and non-valid-RNA points
            ...
            d = fov_df[i]
            d_filt_rna = np.logical_and(d.channel != bead_channel, d.hamming_distance < max_hamming_dist)
    ...
    #Save merged results
    fname2 = out_folder + exp_name + '_data_summary_simple_plotting_microscope_cleaned.parquet'
    merged_df.loc[:, ['r_px_microscope_stitched', 'c_px_microscope_stitched', 'hamming_distance', 'decoded_genes']].to_parquet(fname2)
    if verbose:
