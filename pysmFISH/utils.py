...
def free_space(hd_path: str, min_free_space: int) -> bool:
    ...
    if free_space_giga <= min_free_space:
        logger.info(
            f"Free space in the HD: {free_space_giga} Gb data cannot be transferred,\
                        not enough space on the HD"
        )
        return False
    else:
        logger.info(
            f"Free space in the HD: {free_space_giga} Gb data can be transferred"
        )
        return True


def end_processing_file(
    path_destination: str, completion_pattern: str = "processing_completed.txt"
):
...
def sort_data_into_folders(experiment_fpath: str, experiment_info: Dict):
    ...
    if len(robofish_logs):
        for log in robofish_logs:
            shutil.move(
                log.as_posix(), (experiment_fpath / "original_robofish_logs").as_posix()
            )
    ...
    if "eel" in experiment_info["Experiment_type"]:
        ...
        if len(nuclei_files) or (len(beads_files)):
            try:
                ...
                os.mkdir(experiment_fpath / "fresh_tissue" / "segmentation")
                os.chmod(experiment_fpath / "fresh_tissue" / "segmentation", 0o777)

            for nuclei in nuclei_files:
                shutil.move(
                    nuclei.as_posix(), (experiment_fpath / "fresh_tissue").as_posix()
                )
                logger.debug(f"moved {nuclei.stem} into fresh tissue")

            for beads in beads_files:
                shutil.move(
                    beads.as_posix(), (experiment_fpath / "fresh_tissue").as_posix()
                )
        ...
        if len(large_views):
            for large_view in large_views:
                shutil.move(
                    large_view.as_posix(), (experiment_fpath / "extra_files").as_posix()
                )
                logger.debug(f"moved {large_view.stem} into extra_files")
        else:
            logger.debug(
                f"The experiment does not have large view images of fresh nuclei"
            )
    ...
    if len(remaining):
        for fremaining in remaining:
            shutil.move(
                fremaining.as_posix(), (experiment_fpath / "extra_files").as_posix()
            )
