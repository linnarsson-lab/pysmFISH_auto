...
class nd2_raw_files_selector(Task):

    """
    Task used to identify the nd2 raw microscopy files generated by
    ...
    def run(self,experiment_fpath: str) -> list:
        """
        ...
        self.logger.debug(f'Number of files to process {len(all_files_to_process)}.')
        return all_files_to_process


class nd2_raw_files_selector_general(Task):
    """
    Task used to identify the .nd2 files in a folder. The files
    ...
    def run(self, folder_fpath: str) -> list:
...
class nikon_nd2_autoparser_zarr(Task):
    """
    Task to parse nikon .nd2 files generated by robofish.
    ...
    def run(self, nd2_file_path, parsed_raw_data_fpath, experiment_info):
        ...
        self.logger.debug(f'processing file {nd2_fname}')
        ...
        self.logger.debug(f'loaded info data file {info_file.stem}')
        
        hybridization_name = info_data['channels']['Hybridization']
        self.logger.debug(f'processing hybridization {hybridization_name}')
        
        try:
            nd2fh = nd2reader.ND2Reader(nd2_file_path)
        except:
            self.logger.error('Cannot load the nd2 file')
            ...
            # self.logger.debug(f'.nd2 file renamed {new_file_name}')
            ...
            # self.logger.debug(f'info data file renamed {new_file_name}')
            # new_file_path = raw_files_dir / new_file_name
...
class nikon_nd2_reparser_zarr(Task):
    """
    This class is used to reparse the raw data stored in the raw_data folder during
    the processing
...
#     logger = prefect_logging_setup("nd2_auto_reparser")
