...
class organise_files_tmp(Task):
    ...
    """
    def run(self,experiment_fpath:str, experiment_info:Dict):
        ...
        try:
            ...
            self.logger.error('missing dark image')
            ...
            try:
                ...
                self.logger.error('missing pipeline env config file')
                ...
                try:
                    ...
                    self.logger.error('missing probes set file')
                    ...
                    if 'barcoded' in experiment_info['Experiment_type']:
                        ...
                        try:
                            ...
                            self.logger.info(f'codebook folder already exist')
                        except FileNotFoundError:
                        ...
                        try:
                            ...
                            self.logger.error('codebook is missing')
                            raise signals.FAIL('codebook is missing')



class sort_data_into_folders(Task):
    """
    Task used to sort the files created by the machine and contained 
...
class create_empty_zarr_file(Task):
    """
    Task that create and empty zarr file 
    ...
    """
    def run(self, experiment_fpath:str,tag:str)-> str:
...
class consolidate_zarr_metadata(Task):
    """
    Task to consolidate all the zarr metadata in one unique
    ...
    def run(self, parsed_raw_data_fpath):
        """
        ...
        try:
            ...
            self.logger.error(f'cannot consolidate metadata of the parsed zarr file')
            signals.FAIL(f'cannot consolidate metadata of the parsed zarr file')
        else:
            return consolidated_grp


class sorting_grps(Task):
    """
    ...
    def run(self,grps, experiment_info, analysis_parameters):
        """
        Function used to separate the group names according to 
        the processing type
        Args:
